...
class TestGetListUsers:
    ...
    def test_list_users_should_return_success(self, base_reqres_url):
        ...
        ), f"Response should have status 200, but got {response.status_code}"
    ...
    def test_list_users_response_should_contain_correct_values(self, base_reqres_url):
        ...
        assert response_data.get("page") == pages.get(
            "page"
        ), f"Attribute 'page' should have value {pages.get('page')}, but got {response_data.get('page')}"
        assert (
            response_data.get("per_page") == 6
        ), "Attribute 'per_page' should have value 6"
        assert (
            response_data.get("total") == 12
        ), "Attribute 'total' should have value 12"
        assert (
            response_data.get("total_pages") == 2
        ), "Attribute 'total_pages' should have value 2"
        ...
        def test_list_users_data_integrity(base_reqres_url):
            ...
            assert len(ids) == len(
                set(ids)
            ), "IDs are not unique within the returned data"
            assert response_data["total"] == len(
                response_data["data"]
            ), "Total attribute is not consistent with the number of items returned"
            assert (
                response_data["total_pages"] == 1 or response_data["total_pages"] == 2
            ), "Total pages attribute is not consistent with the data returned"
        ...
        def test_list_users_pagination_first_page(base_reqres_url):
            ...
            assert (
                response_data_first["page"] == 1
            ), "First page request should return page 1"
        ...
        def test_list_users_pagination_last_page(base_reqres_url):
            ...
            assert (
                response_data_last["page"] == 2
            ), "Last page request should return page 2"
        ...
        def test_list_users_pagination_non_existing_page(base_reqres_url):
            ...
            assert (
                response.status_code == 200
            ), "Requesting a page beyond the total number of pages should still return status 200"
            assert (
                response.json()["data"] == []
            ), "Requesting a page beyond the total number of pages should return an empty data array"
